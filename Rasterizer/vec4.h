...
public:
    ...
    void normalise() {
        if (vec4Opti) {
            ...
            __m128 vlen = _mm_set_ss(len2);
            __m128 inv = _mm_rsqrt_ss(vlen);
            float invLen = _mm_cvtss_f32(inv);
            x *= invLen;
            y *= invLen;
            z *= invLen; // Optimised version. Times by invLen instead of dividing by Len. Also using SIMD for square root and division.
        }
        else {
