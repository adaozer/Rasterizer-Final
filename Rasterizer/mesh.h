...
public:
    ...
    void addVertex(const vec4& vertex, const vec4& normal) {
        ...
        if (meshOpti) {
            vertices.emplace_back(v); //Emplace back instead of push_back to save some memory
        }
        else {
    ...
    static Mesh makeCube(float size) {
        ...
        if (meshOpti) { // Clear the pre-existing elements in the vectors and reserve enough memory for what we're going to put in.
            mesh.vertices.clear();
            mesh.triangles.clear();
    ...
    static Mesh makeSphere(float radius, int latitudeDivisions, int longitudeDivisions) {
        ...
        if (meshOpti) { // Reserve again but this time we have to calculate numTriangles and numVertices.
            int numVertices = (latitudeDivisions + 1) * (longitudeDivisions + 1);
            int numTriangles = latitudeDivisions * longitudeDivisions * 2;
